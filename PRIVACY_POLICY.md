# Privacy Policy (HyperYap)

**Last updated:** March 16, 2026

HyperYap is a privacy-first, open-source speech-to-text desktop application. It is designed to work without an internet connection and to keep your speech data on your device.

## Summary

- HyperYap does **not** include analytics or telemetry.
- Your audio and transcriptions are processed **locally** on your device.
- HyperYap stores only what it needs to operate (settings, custom words/rules). No transcription is written to disk by default.
- On startup, HyperYap checks for updates by contacting the configured updater endpoint (currently hosted on GitHub). Other network features are opt-in (downloading updates, enabling the local HTTP API, or connecting to an LLM endpoint you configured).
...
HyperYap stores the following locally in the app's data directory:
...
HyperYap allows exporting and importing settings as `.hyperyap` files. These files may contain: system settings, shortcuts, formatting rules, LLM Connect configuration (endpoint URLs, modes, prompts), and dictionary words.
...
HyperYap may access the network only for:
...
To insert transcriptions into other applications, HyperYap may:
...
Clipboard content is not transmitted by HyperYap.
